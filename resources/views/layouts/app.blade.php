...
<body x-data="{ 
    ...
    <aside x-show="mobileOpen" 
           ...
           class="fixed top-0 left-0 bottom-0 z-50 w-72 bg-white flex flex-col pt-5 pb-6 shadow-2xl md:hidden"
           style="display: none;">
        
        <div class="px-5 mb-5 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#3525cd] to-[#4f46e5] flex items-center justify-center text-white shadow-md">
                    <span class="material-symbols-outlined text-xl">phonelink_setup</span>
                </div>
                <div>
                    <h2 class="text-base font-black text-[#3525cd] tracking-tight">ContractFlow</h2>
                    <p class="text-[11px] text-[#505f76] font-medium">携帯回線・乗り換え管理</p>
                </div>
            </div>
        ...
        <div class="flex-1 px-3 flex flex-col gap-1 overflow-y-auto">
            <a href="{{ route('dashboard') }}" 
               @click="mobileOpen = false"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-sm font-medium transition-all {{ request()->routeIs('dashboard') ? 'bg-[#d0e1fb]/70 text-[#0b1c30] font-bold shadow-xs' : 'text-[#505f76] hover:bg-[#eaedff] hover:text-[#131b2e]' }}">
            ...
            <a href="{{ route('lines.create') }}" 
               @click="mobileOpen = false"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-sm font-medium transition-all {{ request()->routeIs('lines.create') ? 'bg-[#d0e1fb]/70 text-[#0b1c30] font-bold shadow-xs' : 'text-[#505f76] hover:bg-[#eaedff] hover:text-[#131b2e]' }}">
            ...
            <a href="{{ route('history') }}" 
               @click="mobileOpen = false"
               class="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-sm font-medium transition-all {{ request()->routeIs('history') ? 'bg-[#d0e1fb]/70 text-[#0b1c30] font-semibold' : 'text-[#505f76] hover:bg-[#eaedff] hover:text-[#131b2e]' }}">
                <span class="material-symbols-outlined text-xl {{ request()->routeIs('history') ? 'text-[#3525cd]' : 'text-[#505f76]' }}">history</span>
                <span>乗り換え履歴</span>
            </a>
        </div>

        <div class="px-4 pt-3 border-t border-[#c7c4d8]/40">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full flex items-center gap-3 px-3.5 py-2.5 text-xs font-bold text-[#ba1a1a] hover:bg-[#ffdad6]/40 rounded-xl transition-colors cursor-pointer">
    ...
    <div :class="sidebarExpanded ? 'md:pl-64' : 'md:pl-20'" 
        ...
        <!-- Main Content Area (Responsive padding for Mobile, Tablet, PC) -->
        <main class="w-full max-w-[1600px] mx-auto px-3.5 sm:px-6 md:px-8 lg:px-10 py-4 sm:py-6 md:py-8 pt-18 md:pt-8 pb-24 md:pb-12 flex-1">
            {{ $slot }}
        </main>
    </div>

    <!-- Mobile Bottom Navigation Bar (md:hidden) -->
    <nav class="fixed bottom-0 left-0 w-full z-40 bg-white/95 backdrop-blur-md border-t border-[#c7c4d8]/50 flex items-center justify-around py-1.5 pb-safe px-2 shadow-lg md:hidden">
        <!-- Dashboard Tab -->
        <a href="{{ route('dashboard') }}" 
           class="flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all {{ request()->routeIs('dashboard') ? 'text-[#3525cd] font-bold' : 'text-[#505f76] hover:text-[#131b2e]' }}">
            <span class="material-symbols-outlined text-[22px]">dashboard</span>
            <span class="text-[10px] mt-0.5">ダッシュボード</span>
        </a>

        <!-- Line Create Tab -->
        <a href="{{ route('lines.create') }}" 
           class="flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all {{ request()->routeIs('lines.create') ? 'text-[#3525cd] font-bold' : 'text-[#505f76] hover:text-[#131b2e]' }}">
            <span class="material-symbols-outlined text-[22px]">compare_arrows</span>
            <span class="text-[10px] mt-0.5">乗換・追加</span>
        </a>

        <!-- History Tab -->
        <a href="{{ route('history') }}" 
           class="flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all {{ request()->routeIs('history') ? 'text-[#3525cd] font-bold' : 'text-[#505f76] hover:text-[#131b2e]' }}">
            <span class="material-symbols-outlined text-[22px]">history</span>
            <span class="text-[10px] mt-0.5">乗り換え履歴</span>
        </a>

        <!-- Menu Drawer Trigger -->
        <button @click="mobileOpen = true" 
                class="flex flex-col items-center justify-center py-1 px-3 rounded-xl text-[#505f76] hover:text-[#131b2e] transition-all cursor-pointer">
            <span class="material-symbols-outlined text-[22px]">menu</span>
            <span class="text-[10px] mt-0.5">メニュー</span>
        </button>
    </nav>
