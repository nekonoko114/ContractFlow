<div class="space-y-6 sm:space-y-8 w-full">
    <!-- Toast Message -->
    @if ($toastMessage)
        <div class="fixed top-4 right-4 sm:top-5 sm:right-5 z-50 flex items-center gap-3 bg-emerald-600 text-white px-4 sm:px-6 py-3 rounded-2xl shadow-2xl transition-all duration-300 animate-in fade-in slide-in-from-top-4 max-w-sm sm:max-w-md">
            <span class="material-symbols-outlined text-xl sm:text-2xl shrink-0">check_circle</span>
            <span class="text-xs sm:text-sm font-semibold flex-1">{{ $toastMessage }}</span>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 sm:gap-6 pb-2">
        <div>
            <div class="flex items-center gap-2.5 sm:gap-3">
                <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-tr from-[#3525cd] to-[#4f46e5] flex items-center justify-center text-white shadow-md shrink-0">
                    <span class="material-symbols-outlined text-xl sm:text-2xl">dashboard</span>
                </div>
                <h1 class="text-xl sm:text-2xl md:text-3xl font-black text-[#131b2e] tracking-tight">ダッシュボード</h1>
            </div>
            <p class="text-xs sm:text-sm text-[#505f76] mt-1.5 sm:mt-2 font-medium">ご契約中の携帯電話回線の状況・料金・安全維持期間・乗り換え実績の総合概要</p>
        </div>

        <div class="flex flex-wrap items-center gap-2.5 sm:gap-3 w-full lg:w-auto">
            <!-- CSV Actions (Mobile & Desktop) -->
            <div class="inline-flex items-center gap-2 w-full sm:w-auto">
                <button wire:click="exportCsv" 
                        class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-2xl bg-white border border-[#c7c4d8]/60 text-xs font-bold text-[#131b2e] hover:bg-[#f2f3ff] transition-all shadow-xs cursor-pointer"
                        title="回線一覧をCSVファイルとしてダウンロード">
                    <span class="material-symbols-outlined text-[17px] text-emerald-600">download</span>
                    <span>CSV出力</span>
                </button>
                <button wire:click="openImportModal" 
                        class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-2xl bg-white border border-[#c7c4d8]/60 text-xs font-bold text-[#131b2e] hover:bg-[#f2f3ff] transition-all shadow-xs cursor-pointer"
                        title="CSVファイルから回線情報を一括登録">
                    <span class="material-symbols-outlined text-[17px] text-indigo-600">upload_file</span>
                    <span>一括インポート</span>
                </button>
            </div>
            ...
            <!-- New Contract Button (PC & Mobile) -->
            <a href="{{ route('lines.create') }}" 
               class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#3525cd] hover:bg-[#291cb0] text-white text-xs sm:text-sm font-semibold px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl shadow-md shadow-[#3525cd]/20 active:scale-95 transition-all duration-150">
    ...
    <!-- Bento Grid Summary Cards (Responsive: Mobile 1-col, Tablet 2-col, PC 4-col) -->
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
        <!-- Monthly Total -->
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-[#c3c0ff] transition-all">
            <div class="absolute top-0 right-0 w-24 h-24 sm:w-28 sm:h-28 bg-[#4f46e5]/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110 pointer-events-none"></div>
            <div class="flex items-center justify-between mb-3 sm:mb-4">
                <span class="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#505f76]">月額料金 合計</span>
                <span class="material-symbols-outlined text-xl sm:text-2xl text-[#3525cd] bg-[#f2f3ff] p-1.5 sm:p-2 rounded-xl">payments</span>
            </div>
            <div>
                <div class="flex items-baseline gap-1">
                    <span class="text-2xl sm:text-3xl xl:text-4xl font-black text-[#131b2e] tracking-tight">¥{{ number_format($totalMonthlyFee) }}</span>
                    <span class="text-xs sm:text-sm font-bold text-[#505f76]">/月</span>
                </div>
                <div class="flex items-center gap-2 mt-2 pt-2 border-t border-[#f2f3ff] text-[11px] sm:text-xs text-[#505f76]">
                    <span>年間換算:</span>
        ...
        <!-- Total Data Capacity -->
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-[#c3c0ff] transition-all">
            <div class="absolute top-0 right-0 w-24 h-24 sm:w-28 sm:h-28 bg-[#d0e1fb]/40 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110 pointer-events-none"></div>
            <div class="flex items-center justify-between mb-3 sm:mb-4">
                <span class="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#505f76]">合計 通信容量</span>
                <span class="material-symbols-outlined text-xl sm:text-2xl text-sky-600 bg-sky-50 p-1.5 sm:p-2 rounded-xl">data_usage</span>
            </div>
            <div>
                <div class="flex items-baseline gap-1">
                    <span class="text-2xl sm:text-3xl xl:text-4xl font-black text-[#131b2e] tracking-tight">{{ number_format($totalDataCapacity, 1) }}</span>
                    <span class="text-base sm:text-lg font-black text-[#505f76]">GB</span>
                </div>
                <div class="flex items-center gap-2 mt-2 pt-2 border-t border-[#f2f3ff] text-[11px] sm:text-xs text-[#505f76]">
        ...
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-[#c3c0ff] transition-all">
            <div class="absolute top-0 right-0 w-24 h-24 sm:w-28 sm:h-28 bg-emerald-500/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110 pointer-events-none"></div>
            <div class="flex items-center justify-between mb-3 sm:mb-4">
                <span class="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#505f76]">稼働中の回線数</span>
                <span class="material-symbols-outlined text-xl sm:text-2xl text-emerald-600 bg-emerald-50 p-1.5 sm:p-2 rounded-xl">phonelink_ring</span>
            </div>
            <div>
                <div class="flex items-baseline gap-1">
                    <span class="text-2xl sm:text-3xl xl:text-4xl font-black text-[#131b2e] tracking-tight">{{ $activeLinesCount }}</span>
                    <span class="text-sm sm:text-base font-bold text-[#505f76]">回線</span>
                </div>
                <div class="flex items-center gap-1.5 mt-2 pt-2 border-t border-[#f2f3ff] text-[11px] sm:text-xs text-emerald-700 font-semibold">
                    <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500 inline-block"></span>
                    <span>すべて正常に契約中</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-[#c3c0ff] transition-all">
            <div class="absolute top-0 right-0 w-24 h-24 sm:w-28 sm:h-28 bg-amber-500/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110 pointer-events-none"></div>
            <div class="flex items-center justify-between mb-3 sm:mb-4">
                <span class="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#505f76]">乗り換え累計節約効果</span>
                <span class="material-symbols-outlined text-xl sm:text-2xl text-amber-600 bg-amber-50 p-1.5 sm:p-2 rounded-xl">savings</span>
            </div>
            <div>
                <div class="flex items-baseline gap-1">
                    <span class="text-2xl sm:text-3xl xl:text-4xl font-black text-amber-600 tracking-tight">¥{{ number_format($totalSavings) }}</span>
                    <span class="text-xs sm:text-sm font-bold text-[#505f76]">/年</span>
                </div>
                <div class="flex items-center gap-2 mt-2 pt-2 border-t border-[#f2f3ff] text-[11px] sm:text-xs text-[#505f76]">
    ...
    @if (!empty($carrierStats))
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs space-y-3 sm:space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#3525cd] text-lg sm:text-xl">analytics</span>
                    <h3 class="text-xs sm:text-sm font-bold text-[#131b2e]">キャリア別 契約・乗り換え実績カウント</h3>
                </div>
                <span class="text-[10px] sm:text-xs text-[#505f76]">クリックして該当キャリアで絞り込み</span>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 sm:gap-3">
                @foreach ($carrierStats as $stat)
                    <button wire:click="$set('carrierFilter', '{{ $carrierFilter === $stat['name'] ? '' : $stat['name'] }}')"
                            class="p-3 rounded-2xl border text-left transition-all cursor-pointer flex flex-col justify-between gap-2 {{ $carrierFilter === $stat['name'] ? 'bg-[#eaedff] border-[#3525cd] ring-2 ring-[#3525cd]/20 shadow-xs' : 'bg-[#faf8ff]/70 border-[#c7c4d8]/50 hover:bg-[#f2f3ff]' }}">
                        <div class="flex items-center justify-between">
                            <span class="font-bold text-xs text-[#131b2e] truncate">{{ $stat['name'] }}</span>
                            @if ($stat['active_count'] > 0)
                                <span class="w-2 h-2 rounded-full bg-emerald-500 shrink-0" title="現在利用中"></span>
                            @endif
                        </div>
                        <div class="space-y-0.5 text-[10px] sm:text-[11px]">
    ...
    @if (!empty($phoneStats))
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-4 sm:p-6 shadow-xs space-y-3 sm:space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#3525cd] text-lg sm:text-xl">contact_phone</span>
                    <h3 class="text-xs sm:text-sm font-bold text-[#131b2e]">電話番号別 乗り換え回数・キャリア変遷</h3>
                </div>
                <span class="text-[10px] sm:text-xs text-[#505f76]">電話番号ごとの累計乗り換え回数とこれまでのキャリア履歴</span>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                @foreach ($phoneStats as $pStat)
                    <div class="bg-[#faf8ff] p-3.5 sm:p-4 rounded-2xl border border-[#dae2fd] hover:border-[#c3c0ff] transition-all flex flex-col justify-between gap-3">
                        <div class="flex items-start justify-between">
                            <div>
                                <div class="flex items-center gap-1.5 font-mono font-bold text-xs sm:text-sm text-[#131b2e]">
                                    <span class="material-symbols-outlined text-[#3525cd] text-sm sm:text-base">call</span>
                                    <span>{{ $pStat['phone_number'] }}</span>
                                </div>
                                <div class="text-[11px] sm:text-xs text-[#505f76] mt-0.5 font-medium">{{ $pStat['line_name'] }}</div>
                            </div>
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-bold shrink-0 {{ $pStat['transfer_count'] > 0 ? 'bg-[#eaedff] text-[#3525cd] border border-[#c3c0ff]' : 'bg-slate-100 text-slate-600' }}">
                                <span>{{ $pStat['transfer_count'] > 0 ? '乗り換え ' . $pStat['transfer_count'] . '回' : '新規回線' }}</span>
                            </span>
                        </div>

                        <!-- Carrier Flow Track -->
                        <div class="bg-white p-2 sm:p-2.5 rounded-xl border border-[#c7c4d8]/30 space-y-1">
                            <span class="text-[9px] sm:text-[10px] font-bold text-[#777587] uppercase tracking-wider">キャリア変遷</span>
                            <div class="flex items-center flex-wrap gap-1 sm:gap-1.5 pt-0.5">
                                @foreach ($pStat['carrier_flow'] as $idx => $cName)
                                    <span class="px-1.5 sm:px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-bold {{ $idx === count($pStat['carrier_flow']) - 1 ? 'bg-[#eaedff] text-[#3525cd] border border-[#dae2fd]' : 'bg-[#f2f3ff] text-[#505f76]' }}">
                                        {{ $cName }}
                                    </span>
                                    @if ($idx < count($pStat['carrier_flow']) - 1)
                                        <span class="material-symbols-outlined text-[11px] sm:text-[13px] text-[#777587]">chevron_right</span>
                                    @endif
                                @endforeach
                            </div>
                        </div>

                        <div class="flex items-center justify-between text-[11px] sm:text-xs pt-1 border-t border-[#dae2fd]/60">
    ...
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div class="flex items-center gap-2.5 sm:gap-3">
                <span class="material-symbols-outlined text-[#3525cd] text-xl sm:text-2xl">smartphone</span>
                <h2 class="text-lg sm:text-xl font-bold text-[#131b2e]">契約回線一覧</h2>
                <span class="text-[11px] sm:text-xs font-bold bg-[#eaedff] text-[#3525cd] px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full">
                    {{ $lines->count() }} @if ($totalLinesCount !== $lines->count()) / {{ $totalLinesCount }} @endif 回線
                </span>
            </div>
        </div>

        <!-- Search & Filter Toolbar (Responsive Stack on Mobile, Flex on Desktop) -->
        <div class="bg-white border border-[#c7c4d8]/40 rounded-3xl p-3.5 sm:p-5 shadow-xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 sm:gap-4">
            <!-- Search Bar -->
            <div class="relative flex-1">
                <span class="absolute inset-y-0 left-0 pl-3 sm:pl-3.5 flex items-center pointer-events-none text-[#777587]">
                    <span class="material-symbols-outlined text-[18px] sm:text-[20px]">search</span>
                </span>
                <input wire:model.live.debounce.300ms="search" 
                       type="text" 
                       placeholder="回線名、電話番号、キャリア、プランで検索..."
                       class="w-full pl-9 sm:pl-10 pr-9 sm:pr-10 py-2 sm:py-2.5 bg-[#faf8ff] border border-[#c7c4d8]/60 rounded-2xl text-xs sm:text-sm text-[#131b2e] focus:outline-none focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd] transition-all">
                @if ($search !== '')
                    <button wire:click="$set('search', '')" class="absolute inset-y-0 right-0 pr-3 sm:pr-3.5 flex items-center text-[#777587] hover:text-[#131b2e] cursor-pointer">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-wrap items-center gap-2 sm:gap-3">
                <!-- Carrier Filter -->
                <select wire:model.live="carrierFilter" 
                        class="w-full md:w-auto px-3 py-2 sm:px-3.5 sm:py-2.5 bg-[#faf8ff] border border-[#c7c4d8]/60 rounded-2xl text-xs font-semibold text-[#131b2e] focus:outline-none focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                ...
                <select wire:model.live="statusFilter" 
                        class="col-span-2 sm:col-span-1 md:w-auto px-3 py-2 sm:px-3.5 sm:py-2.5 bg-[#faf8ff] border border-[#c7c4d8]/60 rounded-2xl text-xs font-semibold text-[#131b2e] focus:outline-none focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                ...
                @if ($search !== '' || $carrierFilter !== '' || $statusFilter !== '' || $safeFilter !== '')
                    <button wire:click="resetFilters" 
                            class="col-span-2 sm:col-span-3 md:col-span-1 px-3 py-2 sm:px-3.5 sm:py-2.5 text-xs font-bold text-[#ba1a1a] hover:bg-[#ffdad6]/40 rounded-2xl transition-colors flex items-center justify-center gap-1 cursor-pointer">
        ...
        @if ($lines->isEmpty())
            @if ($search !== '' || $carrierFilter !== '' || $statusFilter !== '' || $safeFilter !== '')
                ...
                <div class="bg-white border border-[#c7c4d8]/50 rounded-3xl p-8 sm:p-12 text-center shadow-xs">
                    <div class="w-14 h-14 sm:w-16 sm:h-16 bg-[#f2f3ff] text-[#505f76] rounded-full flex items-center justify-center mx-auto mb-3">
                        <span class="material-symbols-outlined text-2xl sm:text-3xl">search_off</span>
                    </div>
                    <h3 class="text-sm sm:text-base font-bold text-[#131b2e] mb-1">条件に一致する回線が見つかりませんでした</h3>
            ...
            @else
                <div class="bg-white border border-[#c7c4d8]/50 rounded-3xl p-8 sm:p-16 text-center shadow-xs">
                    <div class="w-16 h-16 sm:w-20 sm:h-20 bg-[#eaedff] text-[#3525cd] rounded-full flex items-center justify-center mx-auto mb-4">
                        <span class="material-symbols-outlined text-3xl sm:text-4xl">add_to_queue</span>
                    </div>
                    <h3 class="text-lg sm:text-xl font-bold text-[#131b2e] mb-2">契約回線がまだ登録されていません</h3>
                    <p class="text-xs sm:text-sm text-[#505f76] max-w-md mx-auto mb-6 leading-relaxed">
                        現在お使いの携帯電話会社の回線情報を登録して、月額料金や利用期間の自動計算、乗り換え比較を始めましょう。
                    </p>
                    <a href="{{ route('lines.create') }}" 
                       class="inline-flex items-center gap-2 bg-[#3525cd] hover:bg-[#291cb0] text-white text-xs sm:text-sm font-bold px-5 sm:px-7 py-3 sm:py-3.5 rounded-2xl shadow-lg shadow-[#3525cd]/20 transition-all">
    ...
    @if ($viewingLineHistory && $selectedLine)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
            <div class="bg-white rounded-3xl border border-[#c7c4d8]/50 shadow-2xl max-w-2xl w-full p-5 sm:p-8 max-h-[90vh] overflow-y-auto space-y-5">
                <div class="flex justify-between items-center pb-3 border-b border-[#c7c4d8]/30">
                    <div class="flex items-center gap-2.5">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#eaedff] text-[#3525cd] flex items-center justify-center shrink-0">
                            <span class="material-symbols-outlined text-xl sm:text-2xl">sync_saved_locally</span>
                        </div>
                        <div>
                            <h3 class="text-sm sm:text-lg font-bold text-[#131b2e]">{{ $selectedLine->line_name }} の乗り換え履歴</h3>
                            <p class="text-[11px] sm:text-xs text-[#505f76]">電話番号: {{ $selectedLine->phone_number ?? '未登録' }} | 現在: {{ $selectedLine->carrier_name }}</p>
                        </div>
                    </div>
                    <button wire:click="closeLineHistoryModal" class="p-1 rounded-full text-[#777587] hover:bg-[#f2f3ff] transition-colors cursor-pointer">
                        ...
                        @foreach ($selectedLineHistories as $hist)
                            <div class="bg-[#faf8ff] p-3.5 sm:p-4 rounded-2xl border border-[#dae2fd] space-y-2">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs">
                                    ...
                                    <span class="font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200 self-start sm:self-auto">
                                ...
                                @if ($hist->usage_period_text)
                                    <p class="text-[10px] sm:text-[11px] text-[#777587]">旧回線の利用期間: {{ $hist->usage_period_text }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="flex justify-end pt-2">
                    <button wire:click="closeLineHistoryModal" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#f2f3ff] text-xs font-bold text-[#505f76] hover:bg-[#eaedff] cursor-pointer">
                        閉じる
                    </button>
                </div>
            </div>
    ...
    @if ($isImporting)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
            <div class="bg-white rounded-3xl border border-[#c7c4d8]/50 shadow-2xl max-w-lg w-full p-5 sm:p-8 space-y-4 sm:space-y-5">
                <div class="flex justify-between items-center pb-3 border-b border-[#c7c4d8]/30">
                    <div class="flex items-center gap-2.5">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#eaedff] text-[#3525cd] flex items-center justify-center shrink-0">
                            <span class="material-symbols-outlined text-xl sm:text-2xl">upload_file</span>
                        </div>
                        <h3 class="text-base sm:text-lg font-bold text-[#131b2e]">CSV 一括インポート</h3>
                    </div>
                    <button wire:click="closeImportModal" class="p-1 rounded-full text-[#777587] hover:bg-[#f2f3ff] transition-colors cursor-pointer">
                        <span class="material-symbols-outlined text-2xl">close</span>
                    </button>
                </div>

                <div class="bg-[#faf8ff] p-3 sm:p-4 rounded-2xl border border-[#dae2fd] text-xs text-[#505f76] space-y-1.5 sm:space-y-2">
                    <p class="font-bold text-[#131b2e]">フォーマットについて</p>
                    <p>エクスポートしたCSVと同様の形式で、複数の回線情報を一括登録できます。</p>
                    <p class="text-[10px] sm:text-[11px] text-[#777587]">列順: [ID, 回線名, 電話番号, 携帯会社名, プラン名, 月額料金, 通信容量, 名義人, 使用者, 契約開始日(YYYY-MM-DD), ...]</p>
                </div>

                <form wire:submit="importCsv" class="space-y-4">
                    <div class="border-2 border-dashed border-[#c7c4d8] hover:border-[#3525cd] rounded-2xl p-4 sm:p-6 text-center transition-colors">
                        <input type="file" wire:model="csvFile" accept=".csv,text/csv,text/plain" required class="block w-full text-xs text-[#505f76] file:mr-3 file:py-2 file:px-3 sm:file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-[#eaedff] file:text-[#3525cd] hover:file:bg-[#d0e1fb] cursor-pointer">
                    ...
                    <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-2">
                        <button type="button" wire:click="closeImportModal" class="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-[#c7c4d8] text-xs font-semibold text-[#505f76] hover:bg-[#f2f3ff] cursor-pointer">
                            キャンセル
                        </button>
                        <button type="submit" wire:loading.attr="disabled" class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-[#3525cd] hover:bg-[#291cb0] text-xs font-bold text-white shadow-md cursor-pointer">
                            インポートを実行
                        </button>
                    </div>
                </form>
    ...
    @if ($isEditing)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
            <div class="bg-white rounded-3xl border border-[#c7c4d8]/50 shadow-2xl max-w-2xl w-full p-4 sm:p-8 max-h-[92vh] overflow-y-auto">
                <div class="flex justify-between items-center pb-3 sm:pb-4 border-b border-[#c7c4d8]/30 mb-4 sm:mb-6">
                    <div class="flex items-center gap-2.5">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#eaedff] text-[#3525cd] flex items-center justify-center shrink-0">
                            <span class="material-symbols-outlined text-xl sm:text-2xl">edit_document</span>
                        </div>
                        <h3 class="text-base sm:text-xl font-bold text-[#131b2e]">契約回線情報の編集</h3>
                    </div>
                    <button wire:click="closeEditModal" class="p-1 rounded-full text-[#777587] hover:bg-[#f2f3ff] transition-colors cursor-pointer">
                        <span class="material-symbols-outlined text-2xl">close</span>
                    </button>
                </div>
                ...
                <form wire:submit="updateLine" class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div class="space-y-1.5 sm:col-span-2">
                            ...
                            <input wire:model="edit_line_name" type="text" required class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Phone Number -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">電話番号</label>
                            <input wire:model="edit_phone_number" type="text" placeholder="090-1234-5678" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Carrier Selection -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">携帯会社名 *</label>
                            <select wire:model.live="edit_carrier_name" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                                @foreach ($carriers as $c)
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input wire:model="edit_plan_name" type="text" placeholder="例: ahamo 30GB" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                ...
                                <input wire:model="edit_monthly_fee" type="number" min="0" required class="w-full pl-8 pr-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd] font-bold">
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <input wire:model="edit_data_capacity" type="number" step="0.1" min="0" required class="w-full pr-10 pl-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd] font-bold">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input wire:model="edit_contract_holder" type="text" required class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Actual User -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">使用者 (利用者) *</label>
                            <input wire:model="edit_actual_user" type="text" required class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Network PIN -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">ネットワーク暗証番号 (PIN)</label>
                            <input wire:model="edit_network_pin" type="text" placeholder="4桁の数字など" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm font-mono tracking-widest bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Contract Start Date -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">契約開始日 *</label>
                            <input wire:model="edit_contract_start_date" type="date" required class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Custom Safe Period Days (目標安全日数) -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">目標安全維持日数 (日数)</label>
                            <input wire:model="edit_custom_safe_period_days" type="number" min="0" placeholder="空欄なら標準 (180日/211日)" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60 focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                        </div>

                        <!-- Status -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">利用状況ステータス</label>
                            <select wire:model="edit_status" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-white focus:ring-2 focus:ring-[#3525cd]/20 focus:border-[#3525cd]">
                                <option value="active">契約中 (利用中)</option>
                                <option value="reserved">MNP予約中</option>
                                <option value="transferred">乗り換え済み</option>
                                <option value="cancelled">解約済み</option>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input wire:model="edit_mnp_reservation_number" type="text" placeholder="例: 1234567890" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60">
                        </div>

                        <!-- MNP Expire Date -->
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-[#464555]">MNP有効期限</label>
                            <input wire:model="edit_mnp_reservation_expire_date" type="date" class="w-full px-3.5 py-2.5 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-white">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea wire:model="edit_notes" rows="2" class="w-full px-3.5 py-2 border border-[#c7c4d8]/70 rounded-xl text-xs sm:text-sm bg-[#faf8ff]/60"></textarea>
                    </div>

                    <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-[#c7c4d8]/30">
                        <button type="button" wire:click="closeEditModal" class="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-[#c7c4d8] text-xs sm:text-sm font-semibold text-[#505f76] hover:bg-[#f2f3ff] transition-colors cursor-pointer">
                            キャンセル
                        </button>
                        <button type="submit" class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-[#3525cd] hover:bg-[#291cb0] text-xs sm:text-sm font-semibold text-white shadow-md shadow-[#3525cd]/20 transition-all cursor-pointer">
                            更新を保存する
                        </button>
                    </div>
                </form>
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
            <div class="bg-white rounded-3xl border border-[#c7c4d8]/50 shadow-2xl max-w-md w-full p-5 sm:p-6 text-center">
                <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-3 sm:mb-4">
                    <span class="material-symbols-outlined text-2xl sm:text-3xl">warning</span>
                </div>
                <h3 class="text-base sm:text-lg font-bold text-[#131b2e] mb-2">回線情報を削除しますか？</h3>
                <p class="text-xs text-[#505f76] mb-5 sm:mb-6 leading-relaxed">この操作は取り消せません。該当の回線データが完全に削除されます。</p>
                <div class="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3">
                    <button wire:click="$set('confirmingDelete', false)" class="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-[#c7c4d8] text-xs font-semibold text-[#505f76] hover:bg-[#f2f3ff] cursor-pointer">
                        キャンセル
                    </button>
                    <button wire:click="deleteLine" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#ba1a1a] hover:bg-red-700 text-xs font-semibold text-white shadow-md cursor-pointer">
                        削除を実行する
                    </button>
                </div>
            </div>
